#!/usr/bin/env python3
"""
Visualize Mixed Shift Experiment Results (Additive Target)
Reads from .log files in LocalBash/additiveTgt directory
"""
...
def load_experiment_data(logs_dir='LocalBash/additiveTgt'):
    """Load all AT*.log files"""
    data = {}
    logs_path = Path(logs_dir)
    
    log_files = sorted(logs_path.glob('AT*.log'), key=lambda x: int(re.search(r'(\d+)', x.stem).group(1)))
    
    print(f"\nFound {len(log_files)} AT*.log files:")
    for f in log_files:
        print(f"  - {f.name}")
    ...
    for log_file in log_files:
        exp_match = re.search(r'AT(\d+)\.log', log_file.name)
        if exp_match:
            exp_num = int(exp_match.group(1))
            print(f"[AT{exp_num:2d}] ", end='', flush=True)
...
def plot_comprehensive_analysis(metrics, output_dir='figures'):
    ...
    plt.suptitle(f'Mixed Shift Experiment Results (Additive Target): {n_experiments} Experiments (Ordered by Tgt/Src Ratio)', 
                 fontsize=18, fontweight='bold', y=0.98)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    
    output_file = output_path / 'mixed_shift_experiments_additiveTgt_analysis.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"\n✓ Saved to: {output_file}")
    
    print("\nMapping (Sequential → AT):")
    for i, actual_exp in enumerate(sorted_exp_num):
        if sorted_ratio[i] == float('inf'):
            ratio_str = '∞'
        else:
            ratio_str = f'{sorted_ratio[i]:.2f}'
        print(f"  {i+1:2d} → AT{actual_exp:2d} (ratio: {ratio_str})")
...
def main():
    ...
    print("="*60)
    print("MIXED SHIFT EXPERIMENT VISUALIZER (ADDITIVE TARGET)")
    print("="*60)
    
    data = load_experiment_data('LocalBash/additiveTgt')
    
    if not data:
        print("\n❌ No data found!")
        return
    
    print(f"\n✓ Successfully loaded {len(data)} experiments: AT{sorted(data.keys())}")
