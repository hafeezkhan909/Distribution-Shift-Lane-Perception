...
def draw_latex_style_table(all_data, output_dir):
    ...
    fig.suptitle(
        "Architecture Robustness Summary by Technique",
        fontweight="bold",
        fontsize=14,
        y=0.98,
    )
    ...
    for method in methods:
        ...
        for row in data:
            ...
            weights = [
                ...
                (
                    "bold"
                    if (row["Stable 100%"] == min_100 and min_100 is not None)
                    else "normal"
                ),
                (
                    "bold"
                    if (row["Stable 75%"] == min_75 and min_75 is not None)
                    else "normal"
                ),
                (
                    "bold"
                    if (row["Stable 50%"] == min_50 and min_50 is not None)
                    else "normal"
                ),
                (
                    "bold"
                    if (row["Stable 25%"] == min_25 and min_25 is not None)
                    else "normal"
                ),
            ]
