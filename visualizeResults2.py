...
Reads from .log files in LocalBash/mini directory
"""
...
def load_experiment_data(logs_dir='LocalBash/mini'):
    """Load all CSE*_mini.log files"""
    data = {}
    logs_path = Path(logs_dir)
    
    log_files = sorted(logs_path.glob('CSE*_mini.log'), key=lambda x: int(re.search(r'(\d+)', x.stem).group(1)))
    
    print(f"\nFound {len(log_files)} CSE*_mini.log files:")
    for f in log_files:
        print(f"  - {f.name}")
    ...
    for log_file in log_files:
        exp_match = re.search(r'CSE(\d+)_mini\.log', log_file.name)
        if exp_match:
            exp_num = int(exp_match.group(1))
            print(f"[CSE{exp_num:2d}] ", end='', flush=True)
            ...
            if metrics:
                ...
                # Allow src=0 now (for cases like CSE11)
...
def plot_comprehensive_analysis(metrics, output_dir='figures'):
    ...
    plt.suptitle(f'Mixed Shift Experiment Results (Mini): {n_experiments} Experiments (Ordered by Tgt/Src Ratio)', 
                 fontsize=18, fontweight='bold', y=0.98)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    
    output_file = output_path / 'mixed_shift_experiments_mini_analysis.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"\n✓ Saved to: {output_file}")
    
    print("\nMapping (Sequential → CSE):")
    for i, actual_exp in enumerate(sorted_exp_num):
        if sorted_ratio[i] == float('inf'):
            ratio_str = '∞'
        else:
            ratio_str = f'{sorted_ratio[i]:.2f}'
        print(f"  {i+1:2d} → CSE{actual_exp:2d} (ratio: {ratio_str})")
...
def main():
    ...
    print("="*60)
    print("MIXED SHIFT EXPERIMENT VISUALIZER (MINI)")
    print("="*60)
    
    data = load_experiment_data('LocalBash/mini')
    
    if not data:
        print("\n❌ No data found!")
        return
    
    print(f"\n✓ Successfully loaded {len(data)} experiments: CSE{sorted(data.keys())}")
