...
Reads from .log files in LocalBash directory
"""
...
def load_experiment_data(logs_dir='LocalBash'):
    """Load all CCE log files"""
    data = {}
    logs_path = Path(logs_dir)
    
    # Look for CCE*.log files
    log_files = sorted(logs_path.glob('CCE*.log'), key=lambda x: int(re.search(r'(\d+)', x.stem).group(1))) 
    
    print(f"\nFound {len(log_files)} CCE*.log files:")
    for f in log_files:
        print(f"  - {f.name}")
    ...
    for log_file in log_files:
        # Extract experiment number from filename (e.g., CCE1.log -> 1, CCE10.log -> 10)
        exp_match = re.search(r'CCE(\d+)\.log', log_file.name)
        if exp_match:
            exp_num = int(exp_match.group(1))
            print(f"[CCE{exp_num:2d}] ", end='', flush=True)
            ...
            if metrics:
                ...
                # Allow src=0 now (for edge cases)
...
def plot_comprehensive_analysis(metrics, output_dir='figures'):
    ...
    plt.suptitle(f'Mixed Shift Experiment Results: {n_experiments} Experiments (Ordered by Tgt/Src Ratio)', 
                 fontsize=18, fontweight='bold', y=0.98)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    
    output_file = output_path / 'mixed_shift_experiments_analysis.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"\n✓ Saved to: {output_file}")
    
    print("\nMapping (Sequential → CCE):")
    for i, actual_exp in enumerate(sorted_exp_num):
        if sorted_ratio[i] == float('inf'):
            ratio_str = '∞'
        else:
            ratio_str = f'{sorted_ratio[i]:.2f}'
        print(f"  {i+1:2d} → CCE{actual_exp:2d} (ratio: {ratio_str})")
...
def main():
    ...
    print("="*60)
    print("MIXED SHIFT EXPERIMENT VISUALIZER")
    print("= "*60)
    
    data = load_experiment_data('LocalBash')
    
    if not data:
        print("\n❌ No data found!")
        return
    
    print(f"\n✓ Successfully loaded {len(data)} experiments: CCE{sorted(data.keys())}")
