#!/usr/bin/env python3
"""
Visualize Mixed Shift Experiment Results (Generalized)
Reads from .log files in a specified directory
"""
...
def load_experiment_data(logs_dir, log_pattern='*.log'):
    ...
    # Try to extract experiment numbers from filenames
...
def plot_comprehensive_analysis(metrics, output_dir='figures', folder_name='experiment', epoch_time=None):
    ...
    # Generate filename based on folder name and epoch time
    if epoch_time is None:
        epoch_time = int(time.time())
    
    output_filename = f'{folder_name}_{epoch_time}.png'
    ...
    plt.suptitle(f'Mixed Shift Experiment Results ({folder_name}): {n_experiments} Experiments (Ordered by Tgt/Src Ratio)', 
    ...
    for i, actual_exp in enumerate(sorted_exp_num):
        ...
        print(f"  {i+1:2d} → Exp{actual_exp:2d} (ratio: {ratio_str})")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python visualizeResultsv5.py --log-path LocalBash/additiveTgtMini
  python visualizeResultsv5.py --log-path logs/my_experiment --output-dir results
  python visualizeResultsv5.py --log-path logs/exp1 --pattern "*.log" --epoch-time 1234567890
        """
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Epoch time for output filename (default: current time)'
    )
    
    args = parser.parse_args()
    
    # Extract folder name from log path
    folder_name = Path(args.log_path).name
    
    print("="*60)
    print(f"EXPERIMENT VISUALIZER: {folder_name}")
    print("="*60)
    print(f"Log path: {args.log_path}")
    print(f"Pattern: {args.pattern}")
    print(f"Output dir: {args.output_dir}")
    
    data = load_experiment_data(args.log_path, args.pattern)
